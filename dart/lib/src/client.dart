...
class BluetoothlookupValidationException implements Exception {
  final List<String> errors;

  BluetoothlookupValidationException(this.errors);

  @override
  String toString() => 'BluetoothlookupValidationException: ${errors.join("; ")}';
}
...
/// Bluetooth Company Lookup API client.
///
/// For more information, visit: https://apiverve.com/marketplace/bluetoothlookup?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [companyid] (required) - The Bluetooth SIG company identifier (decimal or hex with 0x prefix). For example, 76 or 0x4C for Apple.
class BluetoothlookupClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'companyid': ValidationRule(type: 'string', required: true),
  };

  BluetoothlookupClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/bluetoothlookup',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [BluetoothlookupValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw BluetoothlookupValidationException(errors);
    }
  }

  /// Execute a request to the Bluetooth Company Lookup API.
  ///
  /// Parameters are validated before sending the request.
  Future<BluetoothlookupResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw BluetoothlookupException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return BluetoothlookupResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw BluetoothlookupException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw BluetoothlookupException('Resource not found');
      } else {
        throw BluetoothlookupException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is BluetoothlookupException) rethrow;
      throw BluetoothlookupException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Bluetooth Company Lookup API client.
class BluetoothlookupException implements Exception {
  final String message;

  BluetoothlookupException(this.message);

  @override
  String toString() => 'BluetoothlookupException: $message';
}
